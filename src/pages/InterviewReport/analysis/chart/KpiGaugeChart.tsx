  ...
  const plugin = useMemo(
    () => ({
      ...
      afterDatasetsDraw(chart: any) {
        ...
        const { left, top, width, height: areaH, right } = chartArea;
        ...
        const labelIndexFromProp = activeLabel
          ? labels.findIndex((label) => label === activeLabel)
          : -1;
        ...
        for (let i = 0; i < n; i++) {
          ...
          if (labelIndexFromProp >= 0) {
            // activeLabel 기준: 해당 칸까지 누적 채움
            if (i < labelIndexFromProp) {
              r = 1;
            } else if (i === labelIndexFromProp) {
              r = forcedRatio;
            } else {
              r = 0;
            }
          } else {
            // fallback: 기존 score/segments 기준
            r = clamp01(segments[i] ?? 0);
          }
        ...
        const displayActiveLabel =
          labels[activeIndex] ?? activeLabel ?? "";
        ...
        if (valueLabel && activeIndex >= 0) {
          const colorTable = {
            ...DEFAULT_VALUE_COLOR_MAP,
            ...(T.valueColorMap || {}),
          };

          const resolvedTextColor =
            colorTable[displayActiveLabel] || T.valueColorFallback;
          ...
          const clampedCenterX = Math.max(
            left + badgeW / 2,
            Math.min(right - badgeW / 2, rawAnchorX)
          );
    ...
    [segments, labels, n, T, L, valueLabel, activeLabel]
  );

  return (
    <div style={{ width: "100%", height }}>
      <Bar
        key={`${valueLabel}-${activeLabel}-${segments.join(",")}-${height}`}
        data={data}
        options={options}
        plugins={[plugin]}
        redraw
