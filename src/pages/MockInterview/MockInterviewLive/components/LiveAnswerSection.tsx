...
export default function LiveAnswerSection() {
  ...
  useEffect(() => {
    ...
    // IMPORTANT: running 상태 변경 시에만 useEffect 실행
    return () => {
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
    };
  }, [running]); // [running]에만 의존하여 무한 재실행 방지
  ...
  const startTimer = () => {
    if (!running) {
        ...
        // startTsRef.current는 useEffect에서 설정됨
        setRunning(true);
    }
  };
  ...
  return (
      ...
      <div className="mock-interview-live__answer">
        ...
        <div className="mock-interview-live__answer-content">
          ...
          >
            ...
            <span className="mock-interview-live__answer-time">
              {minutes} : {seconds}
            </span>
          </div>
    ...
    <div className="mock-interview-dialog__actions">
        <button className="mock-interview-dialog__btn default_btn_white radius"
        onClick={handleMain}>
        메인 페이지로
        </button>
