...
export default function SavedJobPostingSection() {
  ...
  const handleAppliedChanged = (jobId: number, nextApplied: 0 | 1) => {
    setJobs((prev) =>
      prev.map((j) =>
        j.jobIdx === jobId ? ({ ...j, applied: nextApplied } as JobItem) : j
      )
    );
  };
  ...
  const handleFavoriteChanged = (jobId: number, nextFavorite: 0 | 1) => {
    setJobs((prev) =>
      prev.map((j) =>
        j.jobIdx === jobId ? ({ ...j, favorite: nextFavorite } as JobItem) : j
      )
    );
  };
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        console.error("[SavedJobPostingSection] fetch failed:", err);
        setJobs([]);
        setTotalPages(1);
        setTotalCount(0);
      } finally {
